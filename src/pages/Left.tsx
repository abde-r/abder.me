...
const ge = async (slug: any) => {
  const doc = allAbouts.find((doc) => doc.slugAsParams === slug)
  return doc;
}
export default function Left() {
  const content = ge('ff');
  console.log('ff', allAbouts)
  return (
    <div className='flex ml-40 flex-col items-start justify-center p-5 w-[50%] max-md:mx-auto max-md:w-[98%] max-md:pt-20'>
        ...
        {/* <Navbar /> */}
        <div className="mt-20 w-[80%] text-gray-300 max-md:w-[98%] max-md:text-xs">
        <p className=''>{`a `}<span className='font-semibold font-sans text-secondary'>{`0x16 y.o. `}</span>{ allAbouts[0]?.p1 }</p>
          <p className='my-4'>{ allAbouts[0]?.p2 }</p>
        </div>
        {/* <Socials /> */}
    </div>
  )
